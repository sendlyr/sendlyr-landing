...
const allowedEvents = new Set(["page_view", "navigation_click", "workflow_open", "case_study_open", "book_sprint_click", "cohort_toggle", "proof_tab_change", "decision_trace_change", "decision_loop_step", "journey_archetype_change", "delivery_path_change"]);
...
const propertySchemas = {
  ...
  decision_trace_change: new Set(["state"]),
  decision_loop_step: new Set(["step"]),
  journey_archetype_change: new Set(["archetype"]),
  delivery_path_change: new Set(["path"]),
};
...
function validateEvent(event) {
  ...
  if (event.event_name === "decision_trace_change" && !["new_user", "exploring", "at_risk"].includes(event.properties.state)) return "Invalid decision trace state.";
  if (event.event_name === "decision_loop_step" && !["observe", "understand", "decide", "activate", "learn"].includes(event.properties.step)) return "Invalid decision loop step.";
  if (event.event_name === "journey_archetype_change" && !["fast_explorer", "cautious_evaluator", "goal_driven_novice", "assistance_seeking_user"].includes(event.properties.archetype)) return "Invalid journey archetype.";
  if (event.event_name === "delivery_path_change" && !["existing_lifecycle_stack", "managed_email"].includes(event.properties.path)) return "Invalid delivery path.";
